  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      ...
      for (const [id, entry] of Object.entries(parsed as Record<string, unknown>)) {
        if (typeof entry === "string") data[id] = entry;
      }
      return data;
    }
  } catch {
...
export default function WriteScreen() {
  ...
  const selectedValues = useMemo(() => {
    ...
    for (const type of initialTagDataTypes) {
      if (selected.includes(type.id) && values[type.id]?.trim()) {
  ...
  const hasOverflow = initialTagDataTypes.some(
    (type) => selected.includes(type.id) && (values[type.id] ?? "").length > type.maxChar,
  );
  const canWrite = selected.length > 0 && !hasOverflow;
  ...
  function setValue(id: string, value: string) {
    setValues((current) => ({ ...current, [id]: normalizeFieldValue(id, value) }));
  }

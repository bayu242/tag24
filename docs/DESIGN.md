...
* **ONE VERB PER ACTION (mandatory, applies beyond CTAs).** The same underlying action gets the same verb everywhere it appears on the page or across the product, not synonyms picked for variety. If the destroy action is labeled "Delete" in one place, it is "Delete" everywhere, not "Delete" here and "Remove" there and "Trash" somewhere else. Variety in verb choice reads as inconsistency, not as good writing, when the action is identical.
...
- [ ] **One Verb Per Action**: the same action uses the same verb everywhere (no "Delete" here and "Remove" there for the identical action)?
...
- [ ] **Reference Codification** (if a specific reference was supplied, Section 15): rules were measured from the reference, not described from memory? A rebuild-from-rules-alone check was done?

If a single checkbox cannot be honestly ticked, the page is not done. Fix it before delivering.

---

## 15. REFERENCE CODIFICATION (Measure the Reference, Don't Describe It)

Section 0 covers reading a brief that names a vibe, a competitor, or a general direction. This section covers a narrower, harder case: **the user hands you one specific reference** (a screenshot, a saved site, a link to something they love) and says "build something in this style," or "match this," or "make our next five assets look like this one did."

The default failure mode here is **lossy re-description**. The agent looks at the reference, forms a vague verbal impression ("clean, modern, lots of white space, blue accent"), and generates from that impression instead of from the reference itself. Verbal impressions lose the specific ratios, spacing, and restraint that made the original work, and what comes back is a generic cousin of the reference, not a match. This section exists to stop that.

### 15.A The Rule: Measure, Don't Describe

When a specific reference is supplied, do not summarize it into adjectives. Extract concrete, checkable values instead. "Clean and minimal" is not a measurement. "64px section padding, one accent color used on exactly 3 elements, 8px corner radius throughout" is a measurement.

If the environment has an image-analysis or pixel-inspection tool available, use it to pull literal values (hex codes, spacing in px, font sizes, corner radii) rather than eyeballing and guessing. If no such tool is available, look harder and be as literal as your own inspection allows before falling back to a qualitative call, and say so explicitly when you do ("estimated, not measured: this spacing reads as roughly 48-56px").

### 15.B Named Lenses (work through all of them, every time)

Go through the reference against this fixed checklist. Skipping a lens is how the important detail gets missed.

* **Ratios** - proportions between major elements (image-to-text split, header-to-body scale jump, whether spacing follows a consistent scale or a named ratio like the golden ratio).
* **Color split** - what fraction of the composition is neutral vs. accent (a rough 60-30-10 read: dominant / secondary / accent), and exactly which elements carry the accent color.
* **Negative space / refusals** - what the reference deliberately leaves OUT. A tight reference is often defined as much by what's absent (no card borders, no drop shadows, no eyebrow labels) as by what's present. Name the refusals explicitly.
* **Shadows** - present or absent, hard or soft, tinted or neutral, how many distinct shadow treatments appear.
* **Icons and curves** - icon style (line weight, fill vs. outline, corner treatment) and the corner-radius system in use (see Section 4.4's Shape Consistency Lock - the reference has one, find it).
* **Named layout** - does the composition match a pattern already in the Reference Vocabulary (Section 10)? Name it if so ("this is an Asymmetric Split Hero with a Bento Grid feature section"). If it doesn't match a named pattern, describe the structure precisely enough that it could become one.

### 15.C Write the Rules, Then Write a Test Per Rule

Once measured, write the findings down as explicit, falsifiable rules, not impressions. A rule is falsifiable when you could point at a bad copy and say specifically which rule it broke.

For every rule, write a one-line test a bad copy would fail. Examples of the pattern:

* Rule: "Spacing between sections is always 64px, no exceptions." Test: "does any section gap deviate from 64px?"
* Rule: "The accent color appears on exactly the primary CTA and the active nav state, nowhere else." Test: "does the accent color appear on any third element?"
* Rule: "Every card in the grid uses 12px corner radius, matching the button radius." Test: "does any card or button use a different radius?"

A rule that can't be turned into a yes/no test is still an impression, not a codified rule. Push on it until it's checkable.

### 15.D Self-Check: Rebuild From the Rules Alone

Before treating the rule set as finished, try to reconstruct the reference's composition using ONLY the written rules, without looking back at the reference image. Whatever comes out wrong is a rule you forgot to write down or wrote too loosely. Tighten it and repeat. This is the actual validation step, not optional polish: a rule set that can't reproduce the thing it was extracted from is incomplete.

### 15.E Turning a Rule Set Into a Reusable Skill

A reference codified once is a one-off. A reference codified well is reusable. If the user is going to want this look again (a recurring asset type, a brand's ongoing content, a poster series), write the finished rule set down as its own short reference file (name it for what it produces, e.g. `conference-poster.md`, not for the source it came from) with the measured rules and their tests, so it can be pulled back up and applied consistently instead of re-measured from scratch each time.

For a one-off build, the rule set can just live inline in your response as the stated basis for the generated output. For scoring a build against a specific brand's *catalog* design system (as opposed to a one-off supplied reference), that is a different job, handled by DesignMD's `certify_conformance` tool where available, not by this section.

### 15.F Relationship to Section 0

Section 0 (Brief Inference) is for when there is no single concrete reference, just vibe words, audience signals, and general direction. Section 15 is for when there IS a specific reference to match. Use Section 0's Design Read to decide dials and system first; when a concrete reference is also supplied, run it through this section's measurement process before generating, and let the measured rules override the generic dial-driven defaults wherever they conflict.
